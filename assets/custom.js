...
function getScrollbarWidth() {
  ...
  root_styles.style.setProperty(
    '--scrollbar_width',
    window.innerWidth - html_tag.clientWidth + 'px'
  );
}
  ...
  if (nav_main) {
    if (header_outer_id && header_inner) {
      if (header_inner.classList.contains('sticky-nav')) {
        ...
        root_styles.style.setProperty(
          '--sticky_offset',
          nav_main.clientHeight + 'px'
        );
      } else {
        root_styles.style.setProperty(
          '--sticky_offset',
          header_outer_id.clientHeight + 'px'
        );
      }
    }
  }
...
if (content_id && header_inner) {
  ...
  if (ffa !== undefined) {
    ...
    if (
      ffd === true &&
      ffe === true &&
      ffc.classList.contains('wide-transparent')
    ) {
  ...
  if (el.children.length > 1) {
    ...
    if (items.length > parseFloat(minSlides)) {
      ...
      sliderElement.innerHTML =
        '<div class="swiper-outer"><div class="swiper-wrapper">' +
        sliderElement.innerHTML +
        '</div></div> <div class="swiper-custom-pagination"></div>';
      dots_cont = sliderElement.getElementsByClassName(
      ...
      if (settings && settings.pagination) {
        settings.pagination.el =
          settings.pagination.el || '.' + paginationClass;
        dots_cont.appendChild(prev);
        dots_cont.appendChild(dots);
        dots_cont.appendChild(
      ...
      utils: {
        ...
        forms: function () {
          ...
          window.addEventListener('formZindex', function (evt) {
            ...
            form_children = document.querySelectorAll(
          ...
          window.addEventListener('inputPadding', function (evt) {
            ...
            input_prefix = document.querySelectorAll(
            ...
            input_suffix = document.querySelectorAll(
              ...
              Array.from(check_limit).forEach(function (el) {
                if (!el.classList.contains('check-limit-initialized')) {
                  ...
                  if (trigger !== undefined) {
                    nextAll = [].filter.call(
                      trigger.parentNode.children,
                      function (htmlElement) {
                        return htmlElement.previousElementSibling === trigger
                          ? (nextAll = true)
                          : nextAll;
                      }
                    );
                    ...
                    limit.innerText =
                      '+' +
                      Math.abs(
                        el.querySelectorAll('li:not(.hidden, .tip-cont)')
                          .length - el.dataset.limit
                      );
                    if ((tag_limit = 'li')) {
                      limit.innerHTML =
                        '<a href="./">' + limit.innerHTML + '</a>';
                    }
                    el.append(limit);
                    ...
                    Array.from(
                      el.querySelectorAll('a.limit, .limit a')
                    ).forEach(function (em) {
        ...
        top: function () {
          window.addEventListener('top', function (evt) {
            ...
            if (logo_id) {
              ...
              Array.from(logo_id.querySelectorAll('img[alt]')).forEach(
                function (el) {
            ...
            function calcLogoOffset() {
              if (
                ...
                root_styles.style.setProperty(
                  '--logo_offset',
                  (logo_id.offsetLeft / header_id.clientWidth) * 100 + '%'
                );
              }
            }
            ...
            function countNavDist(el, em, nav) {
              var el_off,
                ...
                nav_id_offset = 0;
              (d = el.dataset.copy),
                (f = nav.querySelectorAll(
                  '.show-all li[data-copy="' + d + '"]'
                )[0]);
            ...
            if (navs.length) {
              Array.from(navs).forEach(function (nav_main) {
                ...
                if (nmu !== undefined) {
                  ...
                  if (nml.length) {
                    all_submenu = createElementWithClass(
                      'ul',
                      'show-all-submenu'
                    );
                    ...
                    Array.from(nmt).forEach(function (el) {
                      ...
                      nml[0]
                        .querySelectorAll('.show-all-submenu')[0]
                        .appendChild(clone_me);
                    });
                    ...
                    if (
                      nav_main.closest('#header-inner') !== null &&
                      nav_main
                        .closest('#header-inner')
                        .classList.contains('sticky-nav')
                    ) {
                      function callback(mutationList, observer) {
                        mutationList.forEach(function (mutation) {
                          if (
                            ...
                            var temp_hidden =
                              nav_main.getElementsByClassName('temp-hidden');
                          }
                        });
                      }
            ...
            if (
              nav_id &&
              nav_top_id &&
              !nav_id.querySelectorAll('ul.nav-top').length
            ) {
              Array.from(nav_top_id.querySelectorAll('ul[data-type]')).forEach(
                function (el) {
                  var clone_me = el.cloneNode(true);
            ...
            // Detect empty URLs
            if (nav_main) {
              Array.from(nav_main.querySelectorAll('a[href="#"]')).forEach(
                function (el) {
        ...
        footer: function () {
          ...
          if (footer_id) {
            Array.from(
              ...
              el.querySelector('a.header-toggle').addEventListener(
                'click',
                function (e) {
          ...
          window.addEventListener('background', function (evt) {
            ...
            if (
              background_id &&
              !background_id.classList.contains('static') &&
              (background_id.parentNode.id === 'content' ||
                background_id.parentNode.classList.contains('shopify-section'))
            ) {
            ...
            Array.from(align_middle).forEach(function (el) {
              ...
              if (
                ...
                (el.previousElementSibling === null &&
                  ...
                  em.nextElementSibling === null &&
                  en.id === 'content')
              ) {
                document
                  .getElementById('content')
                  .classList.add('align-center');
              }
            });
          }
          if (align_center.length) {
            Array.from(align_center).forEach(function (el) {
              ...
              if (
                ...
                document
                  .getElementById('content')
                  .classList.add('align-center-static');
              }
            });
          }
          var list_featured_content_box =
                ...
                if (module_tabs.length) {
                  Array.from(module_tabs).forEach(function (el) {
                    if (
                      ...
                      !el.classList.contains('tabs-initialized')
                    ) {
                      semanticTabs(el);
                    }
        ...
        swipers: function () {
          var module_featured,
            ...
            list_usp,
            popup_a,
            list_collection_slider,
            module_collection,
            list_product_slider,
            data_update_product_slider,
            data_update_generic_slider,
            data_scroll_product_slider,
            autoplay_top_int;
              ...
              Array.from(module_featured).forEach(function (el) {
                ...
                featuredSlider = create_slider(el, {
                  ...
                  on: {
                    slideChangeTransitionStart: function (swiper) {
                      Array.from(
                        el.querySelectorAll('.swiper-slide > article.aside')
                      ).forEach(function (em) {
                        em.parentNode.classList.add('has-aside');
                      });
                      var active_content = swiper.el.querySelectorAll(
                      ...
                      if (typeof active_content !== 'undefined') {
                        el.setAttribute(
                          'data-active-content',
                          active_content.getAttribute('class')
                        );
                      }
                ...
                if (featuredSlider !== null) {
                  if (
                    ...
                    el.getElementsByClassName('play-pause')[0].addEventListener(
                      'click',
                      function (e) {
          ...
          window.addEventListener('announcementSlider', function (event) {
            top_bar = document.querySelector(
              '.shopify-section-announcement-bar:not(.s4wi)'
            );
            var top_bar_children = document.querySelectorAll(
              '.shopify-section-announcement-bar:not(.s4wi) > *:not(.close, .overlay-close)'
            );
            if (
              top_bar &&
              top_bar_children.length > 1 &&
              !top_bar.classList.contains('m6kn')
            ) {
              Array.from(
                top_bar.querySelectorAll('.close, .overlay-close')
              ).forEach(function (el) {
                el.remove();
              });
              ...
              create_slider(top_bar, {
                ...
                navigation: {
                  nextEl:
                    '[data-random="' +
                    top_bar.getAttribute('data-random') +
                    '"] .swiper-button-next',
                  prevEl:
                    '[data-random="' +
                    top_bar.getAttribute('data-random') +
              ...
              create_slider(el, {
                ...
                on: {
                  slideChangeTransitionStart: function (swiper) {
                    ...
                    if (swiper.realIndex + 1 === total_sl) {
                      swiper.el[0].parentNode.classList.add(
                        'last-slide-active'
                      );
                    } else {
                      swiper.el[0].parentNode.classList.remove(
                        'last-slide-active'
                      );
                    }
                  },
                  ...
                  on: {
                    slideChangeTransitionStart: function (swiper) {
                      ...
                      if (swiper.realIndex + 1 === total_sl) {
                        ...
                        swiper.el.parentNode.classList.remove(
                          'last-slide-active'
                        );
                      }
                    },
              ...
              Array.from(list_static).forEach(function (el) {
                ...
                if (el.nextElementSibling.classList.contains('mobile-only')) {
                  ...
                  create_slider(el.nextElementSibling, {
                    ...
                    pagination: {
                      el:
                        '.swiper-pagination-' + el.getAttribute('data-random'),
                      clickable: true,
                      type: pagination_type,
                    },
                    navigation: {
                ...
                if (!el.classList.contains('static')) {
                  ...
                  if (!el.classList.contains('l4ch')) {
                    if (el.classList.contains('slider')) {
                      ...
                      if (
                        el.nextElementSibling.classList.contains('l4us-mobile')
                      ) {
          ...
          if (popup_a.length) {
            Array.from(popup_a).forEach(function (el) {
              Array.from(el.getElementsByClassName('l4cl')).forEach(
                function (el) {
          ...
          window.addEventListener('listCollectionSlider', function (event) {
            list_collection_slider = document.querySelectorAll(
              '.l4cl.slider:not(.in-popup, .s4wi)'
            );
            if (list_collection_slider.length) {
              Array.from(list_collection_slider).forEach(function (el) {
                ...
                if (
                  !isNaN(
                    parseFloat(
                      getComputedStyle(el).getPropertyValue('--dist_a')
                    )
                  )
                ) {
                  spacing = parseFloat(
                    ...
                    afterInit: function (swiper) {
                      if (hasImg === true) {
                        el.style.setProperty(
                          '--fih',
                          swiper.el.getElementsByClassName('first-image')[0]
                            .clientHeight + 'px'
                        );
                      }
                ...
                if (hasImg === true && el.closest('li.sub') !== null) {
                  el.closest('li.sub').addEventListener(
                    'mouseenter',
                    function () {
                      el.style.setProperty(
                        '--fih',
                        el.getElementsByClassName('first-image')[0]
                          .clientHeight + 'px'
                      );
                    }
                  );
            ...
            if (list_product_slider.length) {
              Array.from(list_product_slider).forEach(function (el) {
                ...
                var mainSliderElement = el,
                  ...
                  slides,
                  initial_slide = 0,
                  firstModel = el.querySelectorAll(
                    'a > .model-3d:first-child model-viewer[poster]'
                  );
                ...
                if (el.getAttribute('data-featured_media_position')) {
                  initial_slide =
                    parseFloat(
                      el.getAttribute('data-featured_media_position')
                    ) - 1;
                }
                ...
                Array.from(el.getElementsByClassName('m6bx')).forEach(
                  function (em) {
                  ...
                  pagination: {
                    ...
                    renderBullet: function (index, className) {
                      ...
                      if (img !== null) {
                        ...
                        if (a_thumb !== null) {
                          a_thumb_img.setAttribute(
                            'src',
                            a_thumb.getAttribute('data-gallery-thumb')
                          );
                          a_thumb_pic_class =
                            a_thumb.querySelectorAll('picture[class]')[0];
                          if (a_thumb_pic_class) {
                            a_thumb_pic.classList.add(
                              a_thumb_pic_class.getAttribute('class')
                            );
                          }
                      ...
                      if (moreLink) {
                        span = document.createElement('span');
                        span.innerText =
                          '+' + (qttChildren - 1 - index).toString();

                        divFlex.appendChild(span);
                        finalSpan.classList.add('has-more');
                        if (
                          slides[index].querySelectorAll('a[data-fancybox]')
                        ) {
                          Array.from(
                            slides[index].querySelectorAll('a[data-fancybox]')
                          ).forEach(function (em) {
                            finalSpan.setAttribute(
                              'href',
                              em.getAttribute('href')
                            );
                          });
                        }
                      }
                  ...
                  on: {
                    ...
                    activeIndexChange: function () {
                      ...
                      Array.from(
                        ...
                        el.style.width =
                          (100 * (activeIndex + 1)) / qttChildren + '%';
                      });
                    },
                    afterInit: function (swiper) {
                      // create progress bar
                      var progress_bar = createElementWithClass(
                          'div',
                          'custom-progressbar'
                        ),
                        custom_fraction;
                      progress_bar.innerHTML =
                        '<div class="custom-progressbar-inner" style="width:' +
                        100 / qttChildren +
                        '%;"></div>';
                      swiper.el.appendChild(progress_bar);
                      Array.from(
                        swiper.el.querySelectorAll('.s1lb, .label')
                      ).forEach(function (em) {
                        swiper.el.parentNode.appendChild(em);
                      });
                      Array.from(
                        swiper.el.getElementsByClassName('m6bx-inside')
                      ).forEach(function (em) {
                      ...
                      if (
                        ...
                        custom_fraction =
                          swiper.el.parentNode.getElementsByClassName(
                            'swiper-custom-fraction'
                          )[0];
                        custom_fraction.innerHTML =
                          '<span class="swiper-pagination-current">1</span> / <span class="total-el">' +
                ...
                data_update_product_slider =
                  document.querySelectorAll('[data-l4pr-index]');
                data_scroll_product_slider =
                  ...
                  Array.from(data_scroll_product_slider).forEach(function (el) {
                    if (el.tagName.toLowerCase() === 'option') {
                      ...
                      em.addEventListener('change', function () {
                        var dx =
                          em[em.selectedIndex].getAttribute('data-scroll');
                ...
                if (data_update_product_slider.length) {
                  Array.from(data_update_product_slider).forEach(function (el) {
                    if (el.tagName.toLowerCase() === 'option') {
                      ...
                      em.addEventListener('change', function () {
                        var dx =
                          em[em.selectedIndex].getAttribute('data-l4pr-index');
                        if (dx !== null) {
...
if (media_flexible.length) {
  Array.from(media_flexible).forEach(function (el) {
    if (
      !el.parentElement.classList.contains('flexible-stack') &&
      (el.classList.contains('slider-mobile') ||
        el.parentElement.classList.contains('mobile-static'))
    ) {
      var cloned_mobile,
        link,
        emc,
        ...
        fl = pt.querySelectorAll(
          '.media-flexible:not(.mobile-hide-media-flexible) > *:not(.mobile-hide-media-flexible)'
        );
      if (
        !el.parentElement.getElementsByClassName('media-flexible-mobile').length
      ) {
          ...
          if (!cloned_mobile.classList.contains('s4wi')) {
            create_slider(cloned_mobile, {
              ...
              pagination: {
                el:
                  '.swiper-pagination-' +
                  cloned_mobile.getAttribute('data-random'),
                clickable: true,
                type: 'bullets',
                renderBullet: function (index, className) {
                  return (
              ...
              on: {
                slideChangeTransitionStart: function (swiper) {
                  ...
                  if (typeof active_content !== 'undefined') {
                    el.setAttribute(
                      'data-active-content',
                      active_content.getAttribute('data-color-palette')
                    );
                  }
                },
              },
...
window.addEventListener('createCols', function (event) {
  var servicePageElement = document.querySelector(
  ...
  if (servicePageElement != null) {
    if (servicePageElement.parentElement.classList.contains('cols')) {
      ...
      var article =
        servicePageElement.parentElement.querySelector('article.w64.t55');
      if (article) {
        article.replaceWith(...article.childNodes);
      }
      ...
      if (prevSibling) {
        ...
        if (correctSiblingFound) {
          ...
          for (var i = 1; i < wrapperInner.childNodes.length; i++) {
            wrapperInner.insertBefore(
              wrapperInner.childNodes[i],
              wrapperInner.firstChild
            );
          }
          if (anySiblingFound) {
            servicePageElement.parentNode.insertBefore(
              wrapper,
              servicePageElement
            );
...
window.addEventListener(
  ...
  function (event) {
    var messageText = event.detail.message,
      ...
      messageOrigin = event.detail.origin
        ? 'message-' + event.detail.origin
        : false,
      messageColor = '';
    switch (messageType) {
      case 'success':
        messageColor = 'lime';
        if (!messageHeader) {
          messageHeader = window.translations.general_alerts_success_text;
        }
        break;
      case 'info':
        messageColor = 'pine';
        if (!messageHeader) {
          messageHeader = window.translations.general_alerts_info_text;
        }
        break;
      case 'error':
        messageColor = 'rose';
        if (!messageHeader) {
          messageHeader = window.translations.general_alerts_error_text;
        }
        break;
      default:
        messageColor = 'lime';
    ...
    var message =
      ...
      '</p><p>' +
      messageText +
      '</p><a href="./" class="close">Close</a></li>';
    var list_alerts = document.querySelector(
    ...
    if (
      (messageOrigin &&
        list_alerts.getElementsByClassName(messageOrigin).length == 0) ||
      !messageOrigin
    ) {
      // Prevent double messages (multiple of the same forms are being triggered when posted in shopfiy)
...
if (loadMoreItemClicked != null) {
  let el = document.querySelector(
      '#collection > li > figure > a[href="' +
        loadMoreItemClicked +
        '"], .m6cl .results > div a[href="' +
        loadMoreItemClicked +
        '"], .m6cl .results > .l4ne a[href="' +
        loadMoreItemClicked +
        '"]'
    ),
